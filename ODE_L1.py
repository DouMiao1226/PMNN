...
class Model:
    ...
    def Lu_and_F(self):
        x = Variable(self.t, requires_grad=True)
        u_n = self.train_U(x)
    ...
    def PDE_loss(self):
        u_n, F_n = self.Lu_and_F()

        loss = is_cuda(torch.tensor(0.))
        for n in range(1, self.t_N):
            if n == 1:
                pre_Ui = cof * F_n[n] + (C(n - 1) / C(0)) * u_n[0]
            else:
                pre_Ui = is_cuda(cof * F_n[n] + (C(n - 1) / C(0)) * u_n[0])
                for k in range(1, n):
...
def save_loss(i_loss_collect,  f_loss_collect, total_loss):
    ...
    np.savetxt('loss/f_loss_ODE_L1.txt', f_loss_collect)
    np.savetxt('loss/total_loss_ODE_L1.txt', total_loss)
...
def draw_exact_pred():
    ...
    # plt.title('Exact $u(x)$ and Pred $u(x)$ based on $L1$')
    plt.tight_layout()
    plt.tight_layout()
    plt.savefig('Figure/ODE/ODE_L1.png')
    plt.show()
...
def draw_error():
    ...
    plt.yscale('log')
    # plt.title('$Error$ based on $L1$')
    plt.tight_layout()
    plt.savefig('Figure/ODE/ODE_L1_ERROR.png')
    plt.show()
...
def draw_epoch_loss():
    ...
    plt.savefig('Figure/ODE/ODE_L1_LOSS.png')
    plt.show()
...
def draw_epoch_loss_1():
    ...
    plt.savefig('Figure/ODE/ODE_L1_LOSS_1.png')
    plt.show()
