...
class Model:
    ...
    def init_data(self):
        ...
        self.tx_sigma = torch.cat((temp_t + sigma * self.dt, self.x_data), dim=1)
        for i in range(self.t_N - 1):
            ...
            temp_tx_sigma = torch.cat((temp_t + sigma * self.dt, self.x_data), dim=1)
            self.tx_sigma = torch.cat((self.tx_sigma, temp_tx_sigma), dim=0)

        self.tx = is_cuda(self.tx)
        self.tx_sigma = is_cuda(self.tx_sigma)
    ...
    def Lu_and_F(self):
        u_n = self.train_U(self.tx)
        u_n = u_n.reshape(self.t_N, -1)

        x = Variable(self.tx_sigma, requires_grad=True)
        u_n_sigma = self.train_U(x)
        d = torch.autograd.grad(u_n_sigma, x, grad_outputs=torch.ones_like(u_n_sigma),
                                      create_graph=True)
        u_t = d[0][:, [0]]
        u_x = d[0][:, [1]]
        ...
        Lu_sigma = u_xx.reshape(self.t_N, -1)

        return u_n, u_n_sigma, u_t_sigma, Lu_sigma

    def PDE_loss(self):
        u_n, u_n_sigma, u_t_sigma, Lu_sigma = self.Lu_and_F()
        ...
        # for n in range(1, self.t_N):
        #     if n == 1:
        #         pre_lu_sigma = C(n, 0) * (u_n[n] - u_n[n - 1])
        #     else:
        #         pre_lu_sigma = is_cuda(torch.zeros_like(u_n[0]))
        #         for k in range(n):
        #             pre_lu_sigma += C(n, k) * (u_n[n - k] - u_n[n - k - 1])
        #     loss += torch.mean((pre_lu_sigma - cof * (Lu_sigma[n - 1])) ** 2)
...
def save_loss(i_loss_collect, b_loss_collect, f_loss_collect, total_loss):
    ...
    np.savetxt('loss/b_loss_1D_PDE_L2-1_sigma.txt', b_loss_collect)
    np.savetxt('loss/f_loss_1D_PDE_L2-1_sigma.txt', f_loss_collect)
    np.savetxt('loss/total_loss_1D_PDE_L2-1_sigma.txt', total_loss)
...
def draw_exact():
    ...
    plt.tight_layout()
    plt.savefig('Figure/1D_PDE/1D_PDE_L2-1_sigma_exact.png')
    plt.show()
...
def draw_pred():
    ...
    plt.tight_layout()
    plt.savefig('Figure/1D_PDE/1D_PDE_L2-1_sigma_pred.png')
    plt.show()
...
def draw_error():
    ...
    plt.title('Error')
    plt.tight_layout()
    plt.savefig('Figure/1D_PDE/1D_PDE_L2-1_sigma_ERROR.png')
    plt.show()
...
def draw_error_1():
    ...
    plt.title('Error')
    plt.tight_layout()
    plt.savefig('Figure/1D_PDE/1D_PDE_L2-1_sigma_ERROR_1.png')
    plt.show()
...
def draw_some_t(t_num_index):
    ...
    plt.xlabel('$x$')
    plt.ylabel('$u(t,x)$')
    # plt.title('$t = %.2f $' % (t_test[t_num_index]) + ' based on $L2-1$_sigma')
    plt.tight_layout()
    plt.savefig('Figure/1D_PDE/1D_PDE_L2-1_sigma_t_' + str(t_test[t_num_index]) + 'NN_learn.png')
    plt.show()
...
def draw_epoch_loss():
    ...
    plt.savefig('Figure/1D_PDE/1D_PDE_L2-1_sigma_LOSS.png')
    plt.show()
...
def draw_epoch_loss_1():
    ...
    plt.savefig('Figure/1D_PDE/1D_PDE_L2-1_sigma_LOSS_1.png')
    plt.show()
