...
class Model:
    def __init__(self, net, xy, xy_xlb, xy_xub, xy_ylb, xy_yub, t, lb, ub,
        ...
        self.txy_sigma = None #t + sigma * dt时，txy放一起
    ...
    def init_data(self):
        ...
        self.txy_sigma = torch.cat((temp_t + sigma * self.dt, self.xy), dim=1)
        for i in range(self.t_N - 1):
            ...
            temp_txy_sigma = torch.cat((temp_t + sigma * self.dt, self.xy), dim=1)
            self.txy_sigma = torch.cat((self.txy_sigma, temp_txy_sigma), dim=0)

        self.txy = is_cuda(self.txy)
        self.txy_sigma = is_cuda(self.txy_sigma)

        ## 边界条件
        temp_t = torch.full_like(torch.zeros(self.x_N, 1), self.t[0][0])
        temp_t = temp_t + sigma * self.dt
        ...
        self.F_txy_sigma = F(self.txy_sigma)
    ...
    def Lu_and_F(self):
        u_n = self.train_U(self.txy)
        u_n = u_n.reshape(self.t_N, -1)

        x = Variable(self.txy_sigma, requires_grad=True)
        u_n_sigma = self.train_U(x)
        d = torch.autograd.grad(u_n_sigma, x, grad_outputs=torch.ones_like(u_n_sigma), create_graph=True)
        u_t = d[0][:, [0]]
        u_x = d[0][:, [1]]
        u_y = d[0][:, [2]]
        dd_x = torch.autograd.grad(u_x, x, grad_outputs=torch.ones_like(u_x), create_graph=True)
        u_xx = dd_x[0][:, [1]]
        ...
        Lu_sigma = u_xx.reshape(self.t_N, -1) + u_yy.reshape(self.t_N, -1)

        F = self.F_txy_sigma
        F_n_sigma = F.reshape(self.t_N, -1)

        return u_n, u_n_sigma, u_t_sigma, Lu_sigma, F_n_sigma

    def PDE_loss(self):
        u_n, u_n_sigma, u_t_sigma, Lu_sigma, F_n_sigma = self.Lu_and_F()

        loss = is_cuda(torch.tensor(0.))
        for n in range(1, self.t_N):
            if n == 1:
                pre_Ui = u_n[n - 1] + (cof / C(n, 0)) * (Lu_sigma[n - 1] + F_n_sigma[n - 1])
            else:
                pre_Ui = is_cuda(u_n[n - 1] + (cof / C(n, 0)) * (Lu_sigma[n - 1] + F_n_sigma[n - 1]))
                for k in range(1, n):
        ...
        # for n in range(1, self.t_N):
        #     if n == 1:
        #         pre_lu_f_sigma = C(n, 0) * (u_n[n] - u_n[n - 1])
        #     else:
        #         pre_lu_f_sigma = is_cuda(torch.zeros_like(u_n[0]))
        #         for k in range(n):
        #             pre_lu_f_sigma += C(n, k) * (u_n[n - k] - u_n[n - k - 1])
        #
        #     loss += torch.mean((pre_lu_f_sigma - cof * (Lu_sigma[n - 1] + F_n_sigma[n - 1])) ** 2)
    ...
    def LBGFS_loss(self):
        ...
        if self.net.iter % 10 == 0:
            pred_u = self.train_U(txy_test).cpu().detach().numpy()
...
def save_loss(i_loss_collect, b_loss_collect, f_loss_collect, total_loss):
    np.savetxt('loss/i_loss_2D_PDE_L2-1_sigma.txt', i_loss_collect)
    np.savetxt('loss/b_loss_2D_PDE_L2-1_sigma.txt', b_loss_collect)
    np.savetxt('loss/f_loss_2D_PDE_L2-1_sigma.txt', f_loss_collect)
    np.savetxt('loss/total_loss_2D_PDE_L2-1_sigma.txt', total_loss)
...
def exact_draw_some_t(t_num_index):
    ...
    plt.tight_layout()
    plt.savefig('Figure/2D_PDE/2D_PDE_L2-1_sigma_t_' + str(t_test[t_num_index]) + 'NN_exact.png')
    plt.show()
...
def pred_draw_some_t(t_num_index):
    ...
    plt.tight_layout()
    plt.savefig('Figure/2D_PDE/2D_PDE_L2-1_sigma_t_' + str(t_test[t_num_index]) + 'NN_learn.png')
    plt.show()
...
def error_draw_some_t(t_num_index):
    ...
    plt.title('Error')
    plt.tight_layout()
    plt.savefig('Figure/2D_PDE/2D_PDE_L2-1_sigma_t_' + str(t_test[t_num_index]) + 'NN_Error.png')
    plt.show()
...
def error_1_draw_some_t(t_num_index):
    ...
    plt.title('Error')
    plt.tight_layout()
    plt.savefig('Figure/2D_PDE/2D_PDE_L2-1_sigma_t_' + str(t_test[t_num_index]) + 'NN_Error_1.png')
    plt.show()
...
def draw_epoch_loss():
    ...
    plt.xlabel('$Epoch$')
    plt.ylabel('$Loss$')
    # plt.title('$Loss$ based on $L2-1$_sigma')
    ...
    plt.savefig('Figure/2D_PDE/2D_PDE_L2-1_sigma_LOSS.png')
    plt.show()
...
def draw_epoch_loss_1():
    ...
    plt.savefig('Figure/2D_PDE/2D_PDE_L2-1_sigma_LOSS_1.png')
    plt.show()
...
if __name__ == '__main__':
    ...
    # layers = [3, 40, 40, 40, 40, 40, 40, 40, 1]
    # layers = [3, 20, 20, 20, 20, 20, 20, 20, 1]
    # layers = [3, 10, 10, 10, 10, 10, 10, 10, 1]
